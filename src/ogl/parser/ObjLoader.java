...
import ogl.scenegraph.DefaultMesh;
...
public class ObjLoader implements DefaultMesh  { 
	...
	protected static final String[] suffs = new String[] { ".jpg", ".png",
			".gif" };
	...
	public void load(InputStream in) throws IOException {
		...
		try {
				...
				if (line.length() > 0) {
					if (line.startsWith("v ")) {
						...
						if (val.length > 2 && val[2] > -1)
							face_n_ix[0] = val[2];
						vertices.add(new Vertex(p.get(val[0]), c,
								n.get(val[2]), t.get(val[1])));
			...
			positionData = BufferUtils.createFloatBuffer(vertices.size()
					* vecmath.vectorSize());
			colorData = BufferUtils.createFloatBuffer(vertices.size()
					* vecmath.colorSize());
			normalData = BufferUtils.createFloatBuffer(vertices.size()
					* vecmath.vectorSize());
	...
	protected static int[] parseIntTriple(String face) {
		...
		if (ix == -1)
			...
			if (ix2 == -1) {
				return new int[] { Integer.parseInt(face.substring(0, ix)) - 1,
						Integer.parseInt(face.substring(ix + 1)) - 1 };
			} else {
				return new int[] { parseInt(face.substring(0, ix)) - 1,
	...
	public void render(int vertexAttribIdx, int colorAttribIdx,
			int normalAttribIdx, int textureAttribIdx) {
	...
	public void display(Matrix m, Shader shader) { 

		shader.setModelMatrixUniform(m.mult(getTransformation()));
